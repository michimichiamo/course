...
La prima applicazione che esploreremo è la classificazione dei token. Questo termine generico include tutti i problemi che possono essere formulati come "attribuire un'etichetta (*label*) a ogni token in una sequenza", come ad esempio:

- **Named entity recognition (NER)** (*Riconoscimento di entità denominate*): Trovare le entità (come persone, luoghi, o organizzazioni) in una frase. Il problema può essere formulato come quello di attribuire a ogni token un'etichetta che rappresenta il tipo di entità (più un'etichetta speciale per "nessuna entità").
...
- **Chunking** (*Spezzettamento*): Trovare i token che appartengono alla stessa entità. Questo compito (che può essere combinato con POS o NER) può essere formulato come il problema di attribuire un'etichetta ai token che si trovano all'inizio di un frammento (tipicamente `B-` per "beginning", *inizio*), un'altra etichetta ai token che si trovano all'interno di un frammento (tipicamente `I-` per "inside", *dentro*) e una terza etichetta (tipicamente `O`) ai token che non appartengono a nessun frammento.
...
Il modello che addestreremo e caricheremo sull'Hub può essere trovato [qui](https://huggingface.co/huggingface-course/bert-finetuned-ner?text=My+name+is+Sylvain+and+I+work+at+Hugging+Face+in+Brooklyn), e le sue predizioni possono essere verificate!

## Preparazione dei dati

Prima di tutto, abbiamo bisogno di un dataset adatto alla classificazione dei token. In questa sezione utilizzeremo il dataset [CoNLL-2003](https://huggingface.co/datasets/conll2003), che contiene notizie da Reuters. 

<Tip>

💡 Le procedure di pre-elaborazione qui descritte possono essere adattate ad un dataset personale, a condizione che consista di testi suddivisi in parole con le etichette corrispondenti. Per un ripasso su come caricare i propri dati in un `Dataset`, si può fare riferimento al [Capitolo 5](/course/chapter5).

</Tip>

### Il dataset CoNLL-2003

Per caricare il dataset CoNLL-2003, usiamo il metodo `load_dataset()` della libreria 🤗 Datasets:
...
In particolare, notiamo che il dataset contiene etichette per i tre compiti che abbiamo già citato: NER, POS e Chunking. Una differenza rilevante rispetto ad altri dataset consiste nei testi di input, che non sono presentati come frasi o documenti, ma come liste di parole (il nome dell'ultima colonna è `tokens`, ma contiene parole nel senso di input pre-tokenizzati che devono ancora essere processati dal tokenizer attraverso la subword tokenization, *tokenizzazione di sottoparole*).
...
Queste sono le etichette espresse in numeri interi, già pronte per il training, ma non sono immediatamente comprensibili: come nel caso della classificazione testuale, possiamo utilizzare l'attributo `features` del nostro dataset per accedere ai nomi corrispondenti:
...
Come possiamo vedere, gli elementi di questa colonna sono sequenze di `ClassLabel`. Il tipo degli elementi di questa sequenza è contenuto nell'attributo `feature` della variabile `ner_feature`, e possiamo accedere alla lista di nomi attraverso l'attributo `names` di questa stessa `feature`:
